...
function populateSaleAuditFields(query) {
  return query
...
function calculateLineTotal({ quantity, unitPriceSnapshot, discountRateApplied }) {
...
async function loadSaleById(saleId) {
  return populateSaleAuditFields(Sale.findById(saleId));
}
...
async function createSale(payload) {
  ...
  try {
    ...
    if (payload.amountGiven != null && payload.amountGiven < grandTotal) {
      throw createHttpError(400, 'Amount given cannot be less than the grand total.');
    }
    ...
    return loadSaleById(created[0]._id);
  } catch (error) {
...
async function updateSale({ saleId, updates, editedBy }) {
  ...
  if (updates.notes !== undefined) {
    sale.notes = updates.notes || null;
  }
  if (updates.customerName !== undefined) {
    sale.customerName = updates.customerName || null;
  }
  if (updates.customerEmail !== undefined) {
    sale.customerEmail = updates.customerEmail || null;
  }
  ...
  return loadSaleById(sale._id);
}

async function voidSale({ saleId, voidReason, voidedBy }) {
  const session = await mongoose.startSession();
  ...
  try {
    ...
    for (const item of sale.items) {
      for (const allocation of item.allocations) {
        ...
        if (!batch) {
          throw createHttpError(404, 'A referenced batch could not be found while restoring stock.');
        }
